# check for mypki file existence and check that the file has valid config parameters
is_valid_mypki <- function(path) {
  ...
  tryCatch(
    json_data <- jsonlite::fromJSON(txt = path),
    error = function(e) {
  ...
  # verify the Certificate Authority file path that is expected to be in mypki file
